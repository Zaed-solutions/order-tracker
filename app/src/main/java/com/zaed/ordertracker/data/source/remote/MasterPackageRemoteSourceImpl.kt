...
class MasterPackageRemoteSourceImpl(
        ...
        callbackFlow {
            ...
            masterPackagesCollection.where(filter).addSnapshotListener { snapshot, error ->
                if (error != null) {
                    ...
                    Log.d(
                        MasterPackageRemoteSourceImpl::class.simpleName,
                        "${MasterPackageRemoteSourceImpl::getMasterPackagesByGroupId.name}: snapshot: ${snapshot?.documents?.size}",
                    )
                    val masterPackages =
                        snapshot?.documents?.mapNotNull {
                            it.toObject(MasterPackage::class.java)
    ...
    override suspend fun getMasterPackageById(masterPackageId: String): Result<MasterPackage> {
        try {
            ...
            val masterPackage =
                document.toObject(MasterPackage::class.java)
                    ?: return Result.failure(Exception("MasterPackage not found"))

...
sealed interface MasterPackageDetailsUiAction {
    data class OnAddNewShipment(
        val shipment: Shipment,
    ) : MasterPackageDetailsUiAction

    data class OnUpdateSearchQuery(
        val query: String,
    ) : MasterPackageDetailsUiAction

    data class OnEditShipment(
        val shipment: Shipment,
    ) : MasterPackageDetailsUiAction

    data class OnEditMasterPackage(
        val masterPackage: MasterPackage,
    ) : MasterPackageDetailsUiAction

    data class OnDeleteShipment(
